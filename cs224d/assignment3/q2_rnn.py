...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        minus = max_length - len(sentence)
        if minus > 0:
            new_sentence = sentence + [zero_vector] * minus
            new_labels = labels + [zero_label] * minus
            mask = [True]*len(sentence) + [False] * minus
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(tf.int32,\
                [None, self.config.max_length,self.config.n_features])
